...
class buyLogic {
	...
	public function getGoodsList($cart_id, $member_id, $store_id,$orderdiscounts = array()) {
		...
		//会员等级价格  2017年6月23日 17:18:25
		$member_vip = Model('member_seller') -> getMemberSeller(array('buyer_id'=>$member_id,'seller_id'=>$goods_info['store_id']));
		if ($member_vip['vip_id'] > 0) {
			$vip_info = Model('member_vip') -> getMemberVip(array('vip_level_seller_id'=>$goods_info['store_id'],'id'=>$member_vip['vip_id']));
			$goods_price_vip = unserialize($goods_info['goods_price_vip']);
			if ($goods_price_vip) {
				$goods_info['goods_price']= $goods_price_vip[$vip_info['vip_level_name']];
			}
		}
