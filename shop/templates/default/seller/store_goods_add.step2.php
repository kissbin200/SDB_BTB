...
<div class="item-publish">
	<form method="post" id="goods_form" action="<?php if ($output['edit_goods_sign']) { echo urlShop('store_goods_online', 'edit_save_goods');} else { echo urlShop('store_goods_add', 'save_goods');}?>">
		...
		<div class="ncsc-form-goods">
				...
				<dd>
					<?php foreach ($output['vip_list'] as $key => $row) { ?>
						...
						<?php }else{ ?>
							...
							<input name="g_vip_costprice[]" value="" type="text" class="text w60" /><em class="add-on"><i class="icon-renminbi"></i></em><br>
						<?php } ?>
					<?php } ?>
					<span></span>
					<p class="hint">价格必须是0.01~9999999之间的数字</p>
				</dd>
			</dl>
			<dl>
			...
			<dl>
				<dt nc_type="no_spec"><i class="required">*</i>起订量</dt>
				<dd nc_type="no_spec">
					<input name="g_distorage" value="<?php echo $output['goods']['goods_distorage'];?>" type="text" class="text w60" />
					<span></span>
					<p class="hint">商品的最低购买量</p>
				</dd>
			</dl>
			<dl>
			...
			<!-- <h3 id="demo2"><?php echo $lang['store_goods_index_goods_detail_info']?>1111</h3>
