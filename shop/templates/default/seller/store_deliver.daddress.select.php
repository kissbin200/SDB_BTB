		...
		<?php if (is_array($output['address_list']) && !empty($output['address_list'])){?>
			<thead>
				<tr>
					...
					<th><!-- <?php echo $lang['store_deliver_daddress'];?> -->车牌号</th>
			...
			<tbody>
				...
				<tr class="bd-line">
					...
					<td><?php echo $value['company'];?><!--  <?php echo $value['address'];?> --></td>
					<td class="tc"><?php echo $value['telphone'];?></td>
					<td class="tc"><a href="javascript:void(0);" nc_type="select" class="ncbtn" address_id="<?php echo $value['address_id'];?>" address_value="<?php echo $value['seller_name'].'&nbsp;'.$value['telphone'].'&nbsp;'.$value['company'];?>"><?php echo $lang['nc_common_button_select'];?></a></td>
				</tr>
				<?php }?>
				<tr class="bd-line">
					<td colspan="20"></td>
