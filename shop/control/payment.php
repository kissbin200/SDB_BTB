...
class paymentControl extends BaseHomeControl{
	...
	/**
	 * [wxpay_app_notifyOp APP支付成功，更新订单状态]
	 * @return [type] [description]
	 */
	public function wxpay_app_notifyOp(){
		...
		Logs::DEBUG("wxpayorder back:" . json_encode($obj));
	...
	/**
	 * [wxpay_appw_notifyOp APP水币充值支付成功，更新订单状态]
	 * @return [type] [description]
	 */
	public function wxpay_appw_notifyOp(){
		require_once BASE_PATH.'/api/payment/wxpay/log.php';
		...
		$input = file_get_contents('php://input');
		$obj = (array)simplexml_load_string($input, 'SimpleXMLElement', LIBXML_NOCDATA);
		Logs::DEBUG("wxpaywater back:" . json_encode($obj));

		if ($obj['result_code'] == 'SUCCESS' && $obj['return_code'] == 'SUCCESS' ){
			$pdr_info = Model('predeposit') -> getPdRechargeSbInfo(array('pdr_sn'=>$obj['out_trade_no']));
			...
			//修改状态
			$upOrd['pdr_payment_state'] = 1;
			$upOrd['pdr_payment_code'] = $obj['attach'];
			$upOrd['pdr_payment_time'] = time();
			$upOrd['pdr_payment_name'] = '微信充值';
			$upOrd['pdr_trade_sn'] = $obj['transaction_id'];
			Model('predeposit') -> editSbRecharge($upOrd,array('pdr_sn'=>$pdr_info['pdr_sn']));

			//为用户添加水币
			// $water_fee = ($pdr_info['pdr_amount']*100000) + $pdr_info['pdr_amount_fj'];
			$water_fee = $pdr_info['pdr_amount'] + $pdr_info['pdr_amount_fj'];

			$updata['water_fee'] = array('exp','water_fee+'.$water_fee);
			Model('member') -> editMember(array('member_id'=>$pdr_info['pdr_member_id']),$updata);

			//为水厂扣除活动送的水币
			if ($pdr_info['pdr_amount_fj'] > 0) {
				$user = Model('member') -> getMemberInfoByID($pdr_info['pdr_member_id']);

				/*查询用户下有哪些水厂*/
				$storeinfo = Model('store') -> getStoreInfo(array('back_company'=>$user['from_company']));

				//扣除黑锅水厂的水币
				$data_sb['water_fee'] = array('exp','water_fee-'.$pdr_info['pdr_amount_fj']);	
				Model('store') -> editStore($data_sb,array('store_id'=>$storeinfo['store_id']));

				//黑锅水厂水币记录（减少）
				$log_sb_add = array();
				$log_sb_add['log_store_name'] = $storeinfo['store_name'];
				$log_sb_add['log_store_id'] = $storeinfo['store_id'];
				$log_sb_add['log_type'] = 'sb_activity';
				$log_sb_add['log_fee'] = '-'.$pdr_info['pdr_amount_fj'];
				$log_sb_add['log_add_time'] = time();
				$log_sb_add['log_desc'] = "用户参与水币充值活动，订单号: ".$pdr_info['pdr_sn'];
