...
//         Tpl::output('help_list',$help_list);
		...
		$daily_sales = Model('order')->getOrderList($where,'',$field);
		...
		$monthly_sales =Model('order')->getOrderList($where,'',$field);
		Tpl::output('monthly_sales', $monthly_sales);
		unset($field,$where);

		//单品销售排行
		//最近30天getOrderGoodsList
		...
		$where['order_state'] = array('egt',20);
		$ord_arr = Model('order')->getOrderList($where,'','order_id');
		foreach ($ord_arr as $key => $oar) {
			$ord_array[] = $oar['order_id'];
		}
		$wheres['order_id'] = array('in',$ord_array);
		...
		$goods_list = Model('order')->getOrderGoodsList($wheres, $field,'','',$orderby,'goods_name');
