...
class statistics_generalControl extends BaseSellerControl {
	...
	public function generalOp(){
		...
		$statnew_arr['ordernum'] = ($t = $stat_order[0]['ordernum'])?$t:0;
		$statnew_arr['orderamount'] = ncPriceFormat(($t = $stat_order[0]['orderamount'])?$t:(0));
		$statnew_arr['ordermembernum'] = ($t = $stat_order[0]['ordermembernum']) > 0?$t:0;
		$statnew_arr['avgorderamount'] = ncPriceFormat(($t = $stat_order[0]['avgorderamount'])?$t:(0));
		unset($stat_order);
		...
		$where['add_time'] = array('between',array($stime,$etime));
		$field = ' SUM(goods_num) as ordergoodsnum, AVG(goods_pay_price/goods_num) as avggoodsprice ';
		$stat_ordergoods = Model('order') -> getOrderGoodsList($whered,$field);
		$statnew_arr['ordergoodsnum'] = ($t = $stat_ordergoods[0]['ordergoodsnum'])?$t:0;
		$statnew_arr['avggoodsprice'] = ncPriceFormat(($t = $stat_ordergoods[0]['avggoodsprice'])?$t:(0));
		unset($stat_ordergoods);
