	...
	public function member_editOp(){
		...
		if (chksubmit()){
			...
			if ($error != ''){
				...
				$update_array['user_host']      = $_POST['member_user_host'];
		...
		$getAdminInfo = Model('admin') -> getOneAdmin($back_company['id']);

		$member_array['user_host'] = $getAdminInfo['user_host'];
	...
	public function member_addOp(){
		...
		if (chksubmit()){
			...
			$obj_validate->validateparam = array(
				...
				array("input"=>$_POST["member_passwd_pay"], "require"=>"true", "message"=>'支付密码不能为空'),
				array("input"=>$_POST["member_email"], "require"=>"true", 'validator'=>'Email', "message"=>$lang['member_edit_valid_email']),
			...
			if ($error != ''){
				...
				$insert_array['member_paypwd']      = trim($_POST['member_passwd_pay']);
				$insert_array['user_host']      = trim($_POST['member_user_host']);
