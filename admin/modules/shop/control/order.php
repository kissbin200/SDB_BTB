...
class orderControl extends SystemControl{
	...
	private function _get_condition(& $condition) {
		...
		if ($back_company['id']>1) {
			$company['back_company'] = $back_company['id'];

			$memberlist = Model('store') -> getStoreList(array('back_company'=>$company['back_company']));
			foreach ($memberlist as $key => $cid) {
				$sellerid[] = $cid['store_id'];
			}
			$condition['store_id'] = array('in',$sellerid);
		}
